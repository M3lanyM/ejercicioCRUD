...
namespace EjercicioCRUD.Controllers
{
    public class ItemController : Controller
    {
        ...
        public ActionResult ExportarPDF()
        {
            ...
            float[] columnWidths = { 10f, 20f, 25f, 15f, 10f, 10f, 15f, 15f };
            tabla.SetWidths(columnWidths);

            string[] headers = { "ID", "Código", "Descripción", "Precio", "Cantidad", "Proveedor", "Última Venta", "Última Actualización" };
            ...
            foreach (var item in items)
            {
                ...
                tabla.AddCell(item.Precio.ToString("N2")); 
                tabla.AddCell(item.Cantidad.ToString());
                tabla.AddCell(item.IDProveedor.ToString());
                tabla.AddCell(item.UltimaVenta?.ToString("dd/MM/yyyy") ?? "N/A"); 
                tabla.AddCell(item.UltimaActualizacion.ToString("dd/MM/yyyy"));
            }
